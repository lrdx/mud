...
namespace
{
				...
				for (CHAR_DATA *ch : sw.mobs) {
					if (ch && !ch->purged())
					{
						...
						if (boost::starts_with(direction, "”≈◊≈“"))
							dir = SCMD_NORTH;
						if (boost::starts_with(direction, "◊œ”‘œÀ"))
							dir = SCMD_EAST;
						if (boost::starts_with(direction, "¿«"))
							dir = SCMD_SOUTH;
						if (boost::starts_with(direction, "⁄¡–¡ƒ"))
							dir = SCMD_WEST;
						if (boost::starts_with(direction, "◊◊≈“»"))
							dir = SCMD_UP;
						if (boost::starts_with(direction, "◊Œ…⁄"))
							dir = SCMD_DOWN;
	...
	void CheckScheduledRebootCall::perform(int, int)
	{
			...
			&& shutdown_parameters.get_shutdown_timeout() == 0)
		{
			...
			send_to_all("·˜ÙÔÌ·ÙÈ˛ÂÛÎ·Ò ÂÚÂ˙·ÁÚı˙Î· ˛ÂÚÂ˙ 30 ÌÈÓıÙ.\r\n");
	...
	void CheckTriggeredRebootCall::perform(int, int)
	{
		...
		{
			mudlog("Û“¡¬œ‘¡Ã ◊Œ≈€Œ…  ‘“…««≈“ –≈“≈⁄¡«“’⁄À….", DEF, LVL_IMPL, SYSLOG, true);
